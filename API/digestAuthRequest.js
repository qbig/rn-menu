...
function unquote(quotedStr) {
    return quotedStr.substring(1, quotedStr.length-1)
}

function digestAuthRequest(method, url, uri, username, password) {
    ...
    this.formulateResponse = function() {
        var HA1 = CryptoJS.MD5(username+':'+unquote(self.realm)+':'+password).toString();
        console.log(unquote(self.realm))
        console.log(unquote(self.nonce))
        console.log(unquote(self.qop))
